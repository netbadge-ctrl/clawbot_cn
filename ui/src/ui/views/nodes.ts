  ...
  return html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("nodes").pageTitle}</div>
          <div class="card-sub">${t("nodes").pageSubtitle}</div>
        </div>
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const role = req.role?.trim() ? t("nodes").devices.role.replace("{role}", req.role) : t("nodes").devices.role.replace("{role}", "-");
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">
          ${role} · ${t("nodes").devices.requested.replace("{age}", age)}${repair}
        </div>
      </div>
      <div class="list-meta">
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const roles = t("nodes").devices.roles.replace("{list}", formatList(device.roles));
  const scopes = t("nodes").devices.scopes.replace("{list}", formatList(device.scopes));
    ...
    <section class="card">
      ...
      ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            ...
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? t("nodes").loading : t("nodes").bindings.loadConfig}
            </button>
          </div>`
      : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  ...
                  <div class="list-sub">${t("nodes").bindings.defaultBindingSub}</div>
                </div>
                <div class="list-meta">
                  <label class="field">
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        ...
        <button
          ...
          @click=${state.onSave}
        >
          ${state.saving ? t("nodes").approvals.saving : t("nodes").approvals.save}
        </button>
      </div>
      ...
      ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            ...
            <button class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
              ${state.loading ? t("nodes").loading : t("nodes").approvals.load}
            </button>
          </div>`
      : html`
  ...
  return html`
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        ...
        <div class="list-meta">
          <label class="field">
            <span>${t("nodes").approvals.target.host}</span>
            <select
      ...
      if (value === "node") {
        ...
        state.onSelectTarget("gateway", null);
            ...
            >
              <option value="gateway" ?selected=${state.target === "gateway"}>${t("nodes").approvals.target.gateway}</option>
              <option value="node" ?selected=${state.target === "node"}>${t("nodes").approvals.target.node}</option>
            </select>
          </label>
          ${state.target === "node"
      ? html`
                <label class="field">
                  <span>${t("nodes").approvals.target.node}</span>
                  <select
          ...
          state.onSelectTarget("node", value ? value : null);
                  ...
                  >
                    <option value="" ?selected=${nodeValue === ""}>${t("nodes").approvals.target.selectNode}</option>
      ...
      ? html`<div class="muted">${t("nodes").approvals.target.noNodes}</div>`
      : nothing}
    </div>
  `;
}
...
function renderExecApprovalsPolicy(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes").approvals.policy.security}</div>
          <div class="list-sub">
            ${isDefaults
      ? t("nodes").approvals.policy.securityDefault
      : t("nodes").approvals.policy.securityDefaultVal.replace("{value}", defaults.security)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodes").approvals.policy.mode}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
      const target = event.target as HTMLSelectElement;
      ...
      ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    ${t("nodes").approvals.policy.useDefault.replace("{value}", defaults.security)}
                  </option>`
      : nothing}
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes").approvals.policy.ask}</div>
          <div class="list-sub">
            ${isDefaults ? t("nodes").approvals.policy.askDefault : t("nodes").approvals.policy.askDefaultVal.replace("{value}", defaults.ask)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
      ...
      ? html`<option value="__default__" ?selected=${askValue === "__default__"}>
                    ${t("nodes").approvals.policy.useDefault.replace("{value}", defaults.ask)}
                  </option>`
      : nothing}
              ${ASK_OPTIONS.map(
        (option) =>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes").approvals.policy.askFallback}</div>
          <div class="list-sub">
            ${isDefaults
      ? t("nodes").approvals.policy.askFallbackDefault
      : t("nodes").approvals.policy.askFallbackDefaultVal.replace("{value}", defaults.askFallback)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodes").approvals.policy.fallback}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
      const target = event.target as HTMLSelectElement;
      ...
      ? html`<option value="__default__" ?selected=${askFallbackValue === "__default__"}>
                    ${t("nodes").approvals.policy.useDefault.replace("{value}", defaults.askFallback)}
                  </option>`
      : nothing}
              ${SECURITY_OPTIONS.map(
        (option) =>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes").approvals.policy.autoAllow}</div>
          <div class="list-sub">
            ${isDefaults
      ? t("nodes").approvals.policy.autoAllowSub
      : autoIsDefault
        ? t("nodes").approvals.policy.autoAllowDefaultVal.replace("{value}", defaults.autoAllowSkills ? "on" : "off")
        : t("nodes").approvals.policy.autoAllowOverride.replace("{value}", autoEffective ? "on" : "off")}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodes").approvals.policy.enabled}</span>
            <input
              type="checkbox"
              ?disabled=${state.disabled}
      ...
      ? html`<button
              ...
              >
                ${t("nodes").approvals.policy.useDefaultBtn}
              </button>`
      : nothing}
        </div>
      </div>
...
function renderExecApprovalsAllowlist(state: ExecApprovalsState) {
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      ...
      >
        ${t("nodes").approvals.allowlist.addPattern}
      </button>
    </div>
    <div class="list" style="margin-top: 12px;">
      ${entries.length === 0
      ? html`<div class="muted">${t("nodes").approvals.allowlist.noPaths}</div>`
...
function renderAllowlistEntry(
  ...
  const lastUsed = entry.lastUsedAt ? formatAgo(entry.lastUsedAt) : t("nodes").node.never;
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.pattern?.trim() ? entry.pattern : t("nodes").approvals.allowlist.newPattern}</div>
        <div class="list-sub">${t("nodes").approvals.allowlist.lastUsed.replace("{value}", lastUsed)}</div>
        ${lastCommand ? html`<div class="list-sub mono">${lastCommand}</div>` : nothing}
        ${lastPath ? html`<div class="list-sub mono">${lastPath}</div>` : nothing}
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${t("nodes").approvals.allowlist.pattern}</span>
          <input
            type="text"
            .value=${entry.pattern ?? ""}
            ?disabled=${state.disabled}
...
function renderAgentBinding(agent: BindingAgent, state: BindingState) {
  ...
  return html`
    <div class="list-item">
      ...
      ? t("nodes").bindings.usesDefault.replace("{value}", state.defaultBinding ?? t("nodes").bindings.any)
      : t("nodes").bindings.override.replace("{value}", agent.binding!)}
        </div>
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${t("nodes").bindings.binding}</span>
          <select
            ?disabled=${state.disabled || !supportsBinding}
            @change=${(event: Event) => {
      const target = event.target as HTMLSelectElement;
...
function renderNode(node: Record<string, unknown>) {
  ...
  const title =
    (typeof node.displayName === "string" && node.displayName.trim()) ||
    (typeof node.nodeId === "string" ? node.nodeId : t("nodes").node.unknown);
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? t("nodes").node.paired : t("nodes").node.unpaired}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t("nodes").node.connected : t("nodes").node.offline}
          </span>
