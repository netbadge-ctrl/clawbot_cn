  ...
  return html`
    <section class="card">
      ...
      props.onFilterChange((e.target as HTMLInputElement).value)}
        ...
        <div class="muted">${t("skills").shown.replace("{count}", filtered.length.toString())}</div>
      </div>
...
function renderSkill(skill: SkillStatusEntry, props: SkillsProps) {
  ...
  const missing = [
    ...skill.missing.bins.map((b) => `${t("skills").prefixes.bin}:${b}`),
    ...skill.missing.env.map((e) => `${t("skills").prefixes.env}:${e}`),
    ...skill.missing.config.map((c) => `${t("skills").prefixes.config}:${c}`),
    ...skill.missing.os.map((o) => `${t("skills").prefixes.os}:${o}`),
  ];
    ...
    <div class="list-item">
      ...
      ? html`
              <div class="muted" style="margin-top: 6px;">
                ${t("skills").missing.replace("{list}", missing.join(", "))}
              </div>
            `
      : nothing}
        ${reasons.length > 0
      ? html`
              <div class="muted" style="margin-top: 6px;">
                ${t("skills").reason.replace("{list}", reasons.join(", "))}
              </div>
            `
      : nothing}
      </div>
